...
export class AddNewBookDialogComponent {
  ...
  formGroup: FormGroup<WrappedFormControls<IAddBookForm>> = new FormGroup({
  ...
  async addBook(){
    const book = await firstValueFrom(this.dataService.addNewBook(this.formGroup.value as IAddBookForm));
